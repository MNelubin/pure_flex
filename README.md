...
- `buf` — адрес буфера, в который надо записать декодированное сообщение, гарантируется, что размер полезной нагрузки не превосходит `MAX_MESSAGE_LEN`.
- Функция возвращает число считанных в буфер байтов в случае успешной передачи.
  ...
  в стандартный поток ошибок сообщение о возникшей ситуации и вернуть `-1`.
...
- В случае успешной передачи функция возвращает число байтов, записанных в «канал».
...
- [Ссылка на тестирующую систему](https://github.com/spbu-coding-2025/3-grading-system)
